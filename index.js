...
if (multi){
		    var prefix = /^[°zZ#$@*+,.?=''():?%!¢£¥??¤??_&><`?©®?^??¦|/\\©^]/.test(cmd) ? cmd.match(/^[°zZ#$@*+,.?=''():?%¢£¥??¤??_&><!`?©®?^??¦|/\\©^]/gi) : '.'
        } else {
            if (nopref){
                prefix = ''
            } else {
...
const body = (type === 'listResponseMessage' && mek.message.listResponseMessage.title) ? mek.message.listResponseMessage.title : (type === 'buttonsResponseMessage' && mek.message.buttonsResponseMessage.selectedButtonId) ? mek.message.buttonsResponseMessage.selectedButtonId : (type === 'conversation' && mek.message.conversation.startsWith(prefix)) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption.startsWith(prefix) ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption.startsWith(prefix) ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text.startsWith(prefix) ? mek.message.extendedTextMessage.text : (type == 'stickerMessage') && (getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== null && getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) !== undefined) ? getCmd(mek.message.stickerMessage.fileSha256.toString('base64')) : ""
    ...
    const botNumberss = ????????.user.jid + "@c.us";
...
// ???????? ???????? ????
var ampun = await ????????.chats.array.filter(v => v.jid.endsWith('g.us'))
ampun.map( async ({ jid }) => {
if (readGc === false) return
await ????????.chatRead(jid)
})
// ???????? ???????? ????
var chatss = await ????????.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
await ????????.updatePresence(from, Presence.recording)
} else if (autoketik) {
...
const checkRegisteredUser = (sender) => {
    ...
    let senderr = mek.key.fromMe
      ...
      ? mek.participant
      : mek.key.remoteJid;
    const totalchat = await ????????.chats.all();
    const ownerNumber = [`919337956084@s.whatsapp.net`]
	const senderNumber = sender.split("@")[0] 
    const m = simple.smsg(????????, mek);
   ...
   const isRegister = register.includes(sender)
    ...
    const conts = mek.key.fromMe
      ? ????????.user.jid
      : ????????.contacts[sender] || { notify: jid.replace(/@.+/, "") };
...
cmhit.push(command)
  ...
  /*const reply = (teks) => {
      ????????.sendMessage(from, teks, text, { quoted: mek });
    };*/
    const textImg = (teks) => {
...
/*const reply = (teks) => {
			????????.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919337956084?text=hello`}}})
		}*/
    ...
    const mentions = (teks, memberr, id) => {
      id == null || id == undefined || id == false
        ? ????????.sendMessage(from, teks.trim(), extendedText, {
...
download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            ????????.sendMessage(to, media, MessageType.sticker, { quoted: mek });
...
if (budy.toLowerCase() === `8473`){
		if (isRegister) return 
		    ...
		    ????????.sendMessage(from, teks, text, {quoted: fkontak })
...
????????.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await ????????.prepareMessage(from, kma, image)
...
????????.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await ????????.prepareMessage(from, kma, video)
const buttonMessages = {
...
????????.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      ????????.groupAdd(from, orangnya);
    };
        const grupinv = (teks) => {
        	grup = ????????.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `???????? ?????? Inc.`, "footerText": "*_???????? ??????_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
            ????????.relayWAMessage(grup)
        }
const fakeitem = (teks) => {
            ????????.sendMessage(from, teks, text, {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`???????? ??????`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
             res = ????????.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 111119999, "message": teks, "footerText": "*????????*", "thumbnail": bgg, "surface": 'CATALOG' }}, {quoted:ftrol})
             ????????.relayWAMessage(res)
        }
const fakewa = (teks) => {        
????????.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `????????`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/919337956084?text=hello`}}})
		}
const replyfakelink = (teks) => {
????????.sendMessage(from, teks, text,{contextInfo :{text: 'hi',
"forwardingScore": 1000000000,
isForwarded: false,
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\n???????? ??????. (@nnda.krnwn_) ?Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
// Support Di Versi 3.5.2 
    const sendBug = async (target, teks) => {
      ...
      ????????.sendMessage(target, teks, "conversation");
    };
...
async function sendFileFromUrl(from, url, caption, msg, men) {
            ...
            return ????????.sendMessage(from, await getBuffer(url), type, {caption: caption, quoted: mek, mimetype: mime, contextInfo: {"mentionedJid": men ? men : []}})
        }
    ...
    for (let anju of vien){
				if (budy === anju){
					...
					????????.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			//auto sticker
			for (let anji of setik){
				if (budy === anji){
					...
					????????.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			//auto image
			for (let anjh of imagi){
				if (budy === anjh){
					...
					????????.sendMessage(from, result, image, { quoted: mek})
					}
			}
			// auto text 
...
await ????????.setStatus(`Bot Mode On Since ${uptime}`).catch((_) => _);
...
const serialUser = createSerial(18)
	         try {
								ppimg = await ????????.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
*Thank you for registering in the ???????? ?????? Database*
...
*? ???????? ?????? ?*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</?MENU`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(haitod, "imageMessage", { thumbnail: haitod, })).imageMessage
              buttonsMessage = {footerText:'????????', imageMessage: imageMsg,
              contentText:`${anuu}`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
              ????????.relayWAMessage(prep)
	         console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
	    // console.log(e)
            setTimeout( () => {
			????????.updatePresence(from, Presence.composing)
			reply(`*Thank you for being registered ???????? ??????*`)
		}, 2000)
        break
      
case 'help':
case 'menu':
groups = ????????.chats.array.filter(v => v.jid.endsWith('g.us'))
              privat = ????????.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
              ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
           
              uptime = process.uptime();
              timestampe = speed();
              totalChat = await ????????.chats.all()
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       txtt =`Hi ${pushname} ??\n\nIF YOU ARE USING MOD WHATSAPP & BUTTONS DOESNT SHOW, TYPE #allmenu`
               ...
               imageMsg = (await ????????.prepareMessageMedia(fs.readFileSync(`./stik/oksip.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync(`./stik/oksip.jpeg`)})).imageMessage
 ...
 ?????????????? ?????????? : ${privat.length}
 ?????????? ?????????? : ${totalChat.length}
 ?????????? : ${latensie.toFixed(4)} _Second_
 ???????????? : ${runtime(process.uptime())}
 ???????????????? : ${os.platform()}`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fkontak})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        uwuu = '```'
        yoii = '*'
       stst = await ????????.getStatus(`${sender.split('@')[0]}@c.us`)
...
???? ?????? ?? ???????????????????? ??????
`
sendButLocation(from, `${menu}`,`?????? ???? @${dtod.split("@")[0]}`, {jpegThumbnail:iye}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`credit`,buttonText:{displayText:'THANKS TO'},type:1},{buttonId:`script`,buttonText:{displayText:'SCRIPT'},type:1}], {contextInfo: { mentionedJid: [ptod,stod]}})
...
case 'credit':
 ...
 ?? : wa.me/${dtod2.split('@')[0]}
 `
     credit = ????????.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1000, "message": `${yez}`, "footerText": "hehe", "thumbnail": iye, "surface": 'CATALOG'}}, {quoted: mek})
            ????????.relayWAMessage(credit)
        break
case 'suit':
...
case 'bugreport':
              ...
              ????????.sendMessage('919337956084@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                ...
                ????????.sendMessage(from, dapuhy, image, {quoted: mek})        
                break    
case 'auu': // by ????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
       reply('No Sis')
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
               ...
               prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{})
               ????????.relayWAMessage(prep)
               break     
...
????????.sendMessage(from, { "degreesLatitude": '', "degreesLongitude": '', "jpegThumbnail": iye}, MessageType.location, {quoted: ikymemek})
           break
case 'mengyoi':
if (!mek.key.fromMe) return;
sendBug(from)
...
case 'buglink':
                if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
                 ????????.toggleDisappearingMessages(from, 0)
               ????????.sendMessage(from, virtex3(prefix), text, { quoted:ftrol, contextInfo :{text: '??',
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
...
????????.sendMessage(from, listMsg, MessageType.listMessage, {quoted:ftrol})
????????.toggleDisappearingMessages(from, 0)
break
case 'bugcatalog':
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
????????.toggleDisappearingMessages(from, 0)
 babi = (teks) => {
             res = ????????.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 9999999, "message": teks, "footerText": `${emoji2(prefix)}`, "thumbnail": iye, "surface": 'CATALOG' }}, {quoted: {
  key: {
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
                                    if (error) return reply(mess.error.api)
									 ????????.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: freply})
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
		...
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			????????.sendMessage(from, damdu, sticker, {quoted: mek})
			break
 case 'owner3':
let ini_list = []
for (let i of nomorowner1) {
const vname = ????????.contacts[i] != undefined ? ????????.contacts[i].vname || ????????.contacts[i].notify : undefined
ini_list.push({
"displayName": '????????',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${????????.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await ????????.sendMessage(from, {
"displayName": `${ini_list.length} contact`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
????????.sendMessage(from,'This is my owners contact',text,{quoted: mek})
break
case 'command':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
 listMsg = {
...
????????.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
case 'downloadmenu':
menu = `?????????????????????? ??????????
?${prefix}????????4 ????????
...
break
case 'catalog':
      list = await ????????.prepareMessageFromContent(from, {
    "listMessage": {
      "title": "PAYMENT",
      "description": "CLICK",
      "buttonText": "",
  ...
  ????????.relayWAMessage(list, {waitForAck: true})
       
case 'triggered':
                
                if (isImage || isQuotedImage) {
                    let encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                    let media = await ????????.downloadAndSaveMediaMessage(encmedia)
                ...
                exec(`webpmux -set exif ./sticker/data.exif ./sticker/triggered.webp -o ./sticker/triggered.webp`, async (error) => {
                    if (error) return reply(mess.error.api)
                    ????????.sendMessage(from, fs.readFileSync(`./sticker/triggered.webp`), sticker, {quoted: freply})
...
case 'script':
		case 'sc':
		case 'sourcecode':
		????????.sendMessage(from, { text: "https://github.com/Sibuuu07/DogeBot", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
if (args.length < 1) return reply('The text?')
                    teks = body.slice(10)
                    ????????.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
  if (args.length < 1) return reply('The text?')
                    teks = body.slice(12)
                    ????????.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${fakeyoi}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await ????????.downloadMediaMessage(encmedia)
						????????.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await ????????.downloadMediaMessage(encmedia)
						????????.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await ????????.downloadMediaMessage(encmedia)
						????????.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					var mediap = await ????????.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    ????????.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					media = await ????????.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    ????????.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    ????????.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Image upload success:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
				return ????????.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
      case "delcmd":
        if (!isQuotedSticker)
...
nm = body.slice(7)
					...
					delb = await ????????.downloadMediaMessage(boij)
					vien.push(`${nm}`)
					fs.writeFileSync(`./media/vn/${nm}.mp3`, delb)
					fs.writeFileSync('./database/vien.json', JSON.stringify(vien))
					????????.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					????????.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					// add sticker
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					????????.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
					case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					????????.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					// add image
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					...
					????????.sendMessage(from, `Success, please check with *${prefix}imagelist*`, MessageType.text, { quoted: mek })
					break
					case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
					try {
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					????????.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					// add respond
					case 'addrespond':
			if (!isOwner && !mek.key.fromMe) return reply('Only owner can use this feature')
              ...
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
                   if (!isBotGroupAdmins) return reply('bot not admin')
					linkgc = await ????????.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\n${groupName} *Group Link*`
					????????.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
                  case 'resetgrouplink':
...
await ????????.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply('this feature is only for group')
     ...
     ????????.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
     break
             case "bc":  
        case "bcimage":  
        case "broadcast":  
...
*[Wait] Wait a moment..*`
				...
				????????.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted:freply, caption: 'Here bro'})
					break  
        //tiktok
        case 'tiktoknowm':   case 'tiktok':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											me = `*Link* : ${a.data}`
											????????.sendMessage(from,meno,MessageType.video,{mimetype:'video/mp4',quoted: mek, caption:me})
											})
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											????????.sendMessage(from,audin,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
											})
										}).catch((err) => reply(`Invalid link`))
									break
					// ml hero
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  sange = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by ?????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  gayy = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
                case 'lesbiancheck':
					// source code by ?????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
                  lesbii = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by ?????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
ganteng = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by ?????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
cantik = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'character':
					// source code by ?????????
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
watak = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
                case 'can':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					????????.sendMessage(from, 'Question : *'+bisakah+'*\n\nAnswer : '+ keh, text, { quoted: mek })
					break
				case 'when':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					...
					????????.sendMessage(from, 'Question : *'+kapankah+'*\n\nAnswer : '+ koh, text, { quoted: mek })
					break
         			  case 'is':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					apakah = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+apakah+'*\n\nAnswer : '+ kah, text, { quoted: mek })
					break
				case 'rate':
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
					rate = body.slice(1)
					...
					????????.sendMessage(from, 'Question : *'+rate+'*\n\nAnswer : '+ te+'%', text, { quoted: mek })
					break
					...
					teks = `The most handsome person in this group is @${akuutc.jid.split('@')[0]} ??`
					jds.push(akuutc.jid)
					...
					teks = `Ciee.. whats happening here @${akuu.jid.split('@')[0]} ???? @${diaa.jid.split('@')[0]} `
					jds.push(akuu.jid)
					jds.push(diaa.jid)
					mentions(teks, jds, true)
					break
                  ...
                  ????????.sendMessage(from, anu, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nhow is it?`, quoted: mek})
                   break
                           case 'wolf':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} ????????*`)
                   F = body.slice(6)
...
if (!isGroup) return reply(`*Only group*`)
             ...
             let online = [...Object.keys(????????.chats.get(ido).presences), ????????.user.jid]
             ????????.sendMessage(from, '*Online list:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
      case "sider":
        ...
        let members = m.quoted.chat.endsWith("g.us")
          ? (await ????????.groupMetadata(m.quoted.chat)).participants.length - 1
          : m.quoted.chat.endsWith("@broadcast")
          ? -1
          : 1;
        let { reads, deliveries } = await ????????.messageInfo(
          m.quoted.chat,
          m.quoted.id
        );
        let txt = `
...
`.trim();
        m.reply(txt, null, {
          contextInfo: {
            mentionedJid: ????????.parseMention(txt),
          },
        });
        break;
      case "q":
        if (!m.quoted) return reply("reply message!");
        let qse = ????????.serializeM(await m.getQuotedObj());
        if (!qse.quoted)
      ...
      case "kick":
        ...
        if (mentioned.length > 1) {
          ????????.groupRemove(from, mentioned);
          reply(mess.success);
        } else if (mentioned.length < 1) {
          anu = mek.message.extendedTextMessage.contextInfo.participant;
          ????????.groupRemove(from, [anu]);
          reply(mess.success);
        } else {
          ????????.groupRemove(from, mentioned);
          reply(mess.success);
...
case 'igstory': 
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    ????????.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    ????????.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
case 'linkwa':
      ...
      case "getbio":
        ...
        var p = await ????????.getStatus(`${yy}`, MessageType.text);
        reply(p.status);
        if (p.status == 401) {
        ...
        case 'getdeskgc':
              if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
				if (!isGroup) return reply('this feature is only for groups')
					anu = from
			   metadete = await ????????.groupMetadata(anu)
				????????.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
      // Get Name
      case "getname":
        var ambl = mek.message.extendedTextMessage.contextInfo.participant;
        const sname =
          ????????.contacts[ambl] != undefined
            ? ????????.contacts[ambl].sname || ????????.contacts[ambl].notify
            : undefined;
        reply(sname);
        break;
      case "setdesc":
        ...
        if (!isBotGroupAdmins) return reply("Bot not admin");
        if (!isGroup) return;
        ????????.groupUpdateDescription(from, `${args.join(" ")}`);
        ????????.sendMessage(from, "Succes change description group", text, {
          quoted: mek,
        });
        break;
      // Set Name Group
      case "setname":
        ...
        ????????.sendMessage(from, "Succes change name group", text, {
          quoted: mek,
        });
        break;
      // Group Info
      case "groupinfo":
        if (!isGroup) return;
        ppUrl = await ????????.getProfilePicture(from); // leave empty to get your own
        buffergbl = await getBuffer(ppUrl);
        ????????.sendMessage(from, buffergbl, image, {
          quoted: mek,
      ...
      case "demote":
        ...
        ????????.groupDemoteAdmin(from, [mentionede]);
      ...
      case "promote":
        ...
        if (
          ...
          return reply("Reply members");
        mentionede = mek.message.extendedTextMessage.contextInfo.participant;
        ????????.groupMakeAdmin(from, [mentionede]);
...
if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  ????????.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Most!')
	  ...
	  ????????.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await ????????.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  ????????.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await ????????.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Most!')
	  for (let i = 0; i < args[0]; i++) {
	  ????????.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
if (Number(oi2) >= 50) return reply('Most!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  ????????.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
      case "revoke":
...
? Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              ????????.sendMessage(from, fs.readFileSync(media), sticker);
              _reminder.splice(
                reminder.getReminderPosition(sender, _reminder),
                1
              );
...
? Type: ${reminder.getReminderType(sender, _reminder)}`,
              ...
              ????????.sendMessage(from, fs.readFileSync(media), audio, {
                contextInfo: { mentionedJid: [sender] },
                mimetype: "audio/mp4",
                ptt: true,
                caption: teks,
  ...
  if (!mek.key.fromMe) return;
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   ????????.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                ????????.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                ????????.groupUpdateDescription(from, `_${pushname} hacked this group_`)             
                await sleep(1000)
                ????????.sendMessage(from, 'Succes Hacked', text, {quoted: fyt})
case 'mygithub':
   ...
   sendButVideo(from, kodo, `Click Next To Continue`, get_result, [                      
          {
            ...
            type: 1,
                    ...
                    break    
case '+91':
                    get_result = await getBuffer(`https://????????.herokuapp.com/api/asupan?apikey=${valkey}`)
                    pll = `Intake`
...
filsize = args[0]
costick = await ????????.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
????????.relayWAMessage(costick)
break

case 'sizeimg':
if (args.length < 1) return reply('Enter the numbers')
filsizei = args[0]
costick3 = await ????????.prepareMessageFromContent(from,{
"imageMessage": {
	"url": m.quoted.url,
	"mimetype": m.quoted.mimetype,
...
*Model* : ${i.phone.device_model}\n\n`;
	...
	case 'chat':
            ...
            var org = pc.split("|")[1];
            ????????.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            reply(`Sukses mengirim chat:\n${org},@${nomor}`)
            break
case 'viewonce':
res = await ????????.prepareMessageFromContent(from,{
"viewOnceMessage": {
"message": {
"imageMessage": {
"mimetype": 'image/jpeg',
      ...
      case "instagram":
        ...
        hx.igdl(args[0]).then(async (result) => {
          for (let i of result.medias) {
            if (i.url.includes("mp4")) {
              let link = await getBuffer(i.url);
              ????????.sendMessage(from, link, video, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            } else {
              let link = await getBuffer(i.url);
              ????????.sendMessage(from, link, image, {
                quoted: mek,
                caption: `Type : ${i.type}`,
              });
            }
...
case 'setprefix':
    ...
    Follower: ${tod.result.user_followers}`;
        ????????.sendMessage(from, buff, image, { quoted: mek, caption: tt });
        break;*/
    ...
    Follower: ${i.result.stats.followerCount}`;
        ????????.sendMessage(from, buff, image, { quoted: mek, caption: ii });
        break;
  ...
  case 'writebook':            
            ...
            reply(mess.wait)
            teks = args.join(" ")
            ini_buf = await getBuffer(`https://????????.herokuapp.com/api/maker/nulis?apikey=${valkey}&text=${teks}`)  
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List??`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(ini_buf, "imageMessage", { thumbnail: ini_buf, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             if (isMedia && !mek.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
             bc = await ????????.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             ????????.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*? ???????? ?????? BROADCASTING ?*\n\n${body.slice(4)}`})
}
...
buttons = [{buttonId: `yuri`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(kon, "imageMessage", { thumbnail: kon, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
buttons = [{buttonId: `anal`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(aku, "imageMessage", { thumbnail: aku, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
buttons = [{buttonId: `lesbian`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(kau, "imageMessage", { thumbnail: kau, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
buttons = [{buttonId: `eroneko`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
buttons = [{buttonId: `bj`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(yoiz, "imageMessage", { thumbnail: yoiz, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
buttons = [{buttonId: `kitsune`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
              ????????.relayWAMessage(prep)
break
case 'pussy':
if (!isGroup) return reply('this feature is only for groups')
...
buttons = [{buttonId: `pussy`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `wallpaper`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `neko2`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `baka`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `slap`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `poke`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
buttons = [{buttonId: `keta`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(hai, "imageMessage", { thumbnail: hai, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: fvn})
...
????????.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./Sibu.jpg')})
break
case  'blowjob':
if (!isGroup) return reply('this feature is only for groups')
if (!isNsfw) return reply(`Nsfw feature is not yet active in this group\nType: ${prefix}nsfw 1 \To activate`)
...
????????.sendMessage(from, anu, text, {quoted: mek})
break
case 'xvideo':
case 'xv': 
...
????????.sendMessage(from, vid, video, {quoted: mek})
break
case 'writelist':
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply('only admin can use this feature')
					...
					if (Number(args[0]) === 1) {
						...
						????????.sendMessage(from, `Free to Find Hentai ??`, text)
			...
			case 'maker3d': 
					...
					????????.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Sibu.jpg')})
					break
			case 'maker3d2': 
                    if (args.length < 1) return reply(`*Example :*\n${prefix}${command} ????????`)
					makell = args.join(" ")
...
buttons = [{buttonId: `quote`,buttonText:{displayText: `Thx Dah Pake`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(buffer1, "imageMessage", { thumbnail: buffer1, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await ????????.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						????????.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:fvn})
				...
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						????????.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: fvid})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ger = isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          owgi = await ????????.downloadAndSaveMediaMessage(ger);
          ????????.sendMessage(sender, fs.readFileSync(owgi), "imageMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
          ...
          ger = isQuotedVideo
            ...
            : mek;
          owgi = await ????????.downloadAndSaveMediaMessage(ger);
          ????????.sendMessage(sender, fs.readFileSync(owgi), "videoMessage", {
            caption: q,
          });
          reply("Success");
          fs.unlinkSync(owgi);
      ...
      case "caripesan":
        ...
        let v = await ????????.searchMessages(q, from, 10, 1);
        let s = v.messages;
        ...
        try {
          ...
          for (let i = 0; i < el.length; i++) {
            await ????????.sendMessage(from, "Here is the message", text, {
              quoted: el[i],
            });
          }
         } catch (e) {
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`;
        ...
        ????????.sendMessage(from, ram, image, { quoted: mek, caption: rem });
        break;
      case "comic":
...
*Final Chapter* : ${komik.chapter_akhir}`;
      ...
      case "chara":
        ...
        await ????????.sendMessage(from, li, image, { quoted: mek });
        break;
      case 'pinterest':
...
buttons = [{buttonId: `pinterest`,buttonText:{displayText: `??Next`},type:1}]
              imageMsg = (await ????????.prepareMessageMedia(di, "imageMessage", { thumbnail: di, })).imageMessage
              buttonsMessage = {footerText:'???????? ??????', imageMessage: imageMsg,
              contentText:`????: ????? = ????s`,buttons,headerType:4}
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: ftrol})
              ????????.relayWAMessage(prep)
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        if (
          ...
          encmedia = isQuotedSticker
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
          file = await ????????.downloadAndSaveMediaMessage(
            encmedia,
            (filename = getRandom())
          );
          value = args.join(" ");
          var group = await ????????.groupMetadata(from);
          ...
          ????????.sendMessage(from, ini_buffer, sticker, options);
          fs.unlinkSync(file);
        } else {
      ...
      case "totag":
        if (
          ...
          ini_buffer = fs.readFileSync(file);
          ????????.sendMessage(from, ini_buffer, image, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          ????????.sendMessage(from, ini_buffer, audio, options);
          fs.unlinkSync(file);
        } else if (
          ...
          ini_buffer = fs.readFileSync(file);
          ????????.sendMessage(from, ini_buffer, video, options);
          fs.unlinkSync(file);
        } else {
          reply(
      ...
      case "slanderpc":
        ...
        const responye = await ????????.sendMessage(
          jids,
          `${split[1]}`,
          MessageType.text,
          options
        );
        await ????????.deleteMessage(jids, {
      ...
      case "tomp3":
        ...
        exec(`ffmpeg -i ${media} ${ran}`, (err) => {
          ...
          ????????.sendMessage(from, buffer453, audio, {
            mimetype: "audio/mp4",
            quoted: mek,
          });
          fs.unlinkSync(ran);
      ...
      case "fast":
        ...
        exec(
          ...
          (err) => {
            ...
            if (err) return fakegroup(`Err: ${err}`);
            buffer453 = fs.readFileSync(ran);
            ????????.sendMessage(from, buffer453, video, {
              mimetype: "video/mp4",
              quoted: mek,
            });
            fs.unlinkSync(ran);
  ...
  case 'buttons5':
          ...
          case 'dare':
              ...
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{contextInfo: { mentionedJid: [sender]},quoted:ftex})
              ????????.relayWAMessage(prep)
              break
      case "anime":
        reply(mess.wait);
...
case 'waifu':
          ...
          case 'wallml':
              ...
              imageMsg = ( await ????????.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              ...
              prep = await ????????.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              ????????.relayWAMessage(prep)
...
Link : ${get_resultP.url_audio}
            `
            ????????.sendMessage(from, textP, text,{contextInfo:{
            "forwardingScore": 1000000000,
            isForwarded: false,
            sendEphemeral: false,
            "externalAdReply": {
            ...
            ????????.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_resultP.title}.mp3`, quoted: mek})
            break
case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} ????????`)
              ...
              break
					...
					????????.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infoearthquake':
					...
					????????.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
      case "contact":
        pe = args.join(" ");
        entah = pe.split("|")[0];
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        require("./lib/fetcher.js").modStick(media, ????????, mek, from);
        break;
      case "stikerwm":
      case "stickerwm":
      case "swm":
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ...
        if ((isMedia && !mek.message.videoMessage) || isQuotedImage) {
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              ????????.sendMessage(from, "Terjadi kesalahan", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
          ...
          ffmpeg(media)
            .on("error", (e) => {
              console.log(e);
              ????????.sendMessage(from, "There is an error", "conversation", {
                quoted: mek,
              });
              fs.unlinkSync(media);
            })
      ...
      case "fdeface":
        ...
        const dipes =
          isQuotedSticker || isQuotedImage
            ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                .extendedTextMessage.contextInfo
            : mek;
        const tipes = await ????????.downloadAndSaveMediaMessage(dipes);
        ...
        var mat = await ????????.generateLinkPreview(url);
        mat.title = title;
        mat.description = desc;
        mat.jpegThumbnail = bufer;
        mat.canonicalUrl = buu;
        ????????.sendMessage(from, mat, MessageType.extendedText, anu);
        break;
      case "public":
        if (!mek.key.fromMe) return reply("This feature is only for the owner ??");
...
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak});
        ...
        ????????.sendMessage(from, optionshidetag, text);
        break;
...
*[Wait] Wait a moment..*`
      ...
      case "s":
        if (
          ...
          await ffmpeg(`./${media}`)
            ...
            .on("end", function () {
              console.log("Finish");
              ????????.sendMessage(from, fs.readFileSync(ran), sticker, {
                quoted: mek,
              });
              fs.unlinkSync(media);
              fs.unlinkSync(ran);
      ...
      case "join":
        try {
          ...
          var response = await ????????.acceptInvite(codeInvite);
          grupinv("SUCCESS");
        } catch {
          ...
          boij =
            isQuotedImage || isQuotedVideo
              ? JSON.parse(JSON.stringify(mek).replace("quotedM", "m")).message
                  .extendedTextMessage.contextInfo
              : mek;
          owgi = await ????????.downloadMediaMessage(boij);
...
exec(q, (err, stdout) => {
if (err) return ????????.sendMessage(from, ` ${err}`, text, { quoted: mek })
if (stdout) {
????????.sendMessage(from, stdout, text, term)
}
})
}
